...
ALL_CROPS_OBS <- c("Corn", "FiberCrop", "MiscCrop", "OilCrop", "OtherGrain", "PalmFruit",  "Rice", "Root_Tuber", "SugarCrop",  "Wheat")
...
crop_nrmse <- read.csv("./2-process/crop_nrmse.csv")
...
ET_COLORS <- c('Hybrid Perfect Adaptive' = '#7fc97f', 'Adaptive' = '#f0027f', 'Linear' = '#beaed4', 'Perfect' = '#fdc086','Hybrid Linear Adaptive' = '#386cb0', 'Obs' = 'black', 'FAO' = 'black', 'CCI' = 'gray45')
...
CALIB_COLORS <- c('Initial Model Year = 1990' = '#f0027f', 'Initial Model Year = 1975' = '#7fc97f', 'Initial Model Year = 2005' = '#beaed4')
...
```{r landcoverchange-ts-sm, echo=FALSE, message=FALSE, fig.width=10, fig.height=6, fig.cap="Land Cover Change over Time by Expectation Type. Black lines are data from the CCI satellite (downloaded from FAOSTAT). Note that all four expectation types produce similar values for Grassland and Shrubland, so these lines are overlapping. "}
best.type <- nrms[which.min(nrms$landTypeMeanObjFunVal), "expectation.type"]
best_output %>%
  filter(calibration.year == 1990,
         comparison.years == "1990-2015",
  ...
  summarize(land.allocation = sum(land.allocation)) %>%
  ungroup() %>%
  filter(GCAM_commodity %in% LANDCOVER_TO_PLOT) ->
  gcam
obs_data %>%
  filter(region == "USA", 
         GCAM_commodity %in% LANDCOVER_TO_PLOT,
...
# Calculate change from 1992, which is the first year of CCI data
gcam %>%
  filter(year == 1992) %>%
  select(GCAM_commodity, land.allocation, scenario) %>%
  rename(base.allocation = land.allocation) ->
  gcam_base
gcam %>%
  select(GCAM_commodity, land.allocation, scenario, year) %>%
  left_join(gcam_base, by=c("GCAM_commodity", "scenario")) %>%
  mutate(diff = land.allocation - base.allocation) ->
  gcam
gcam %>%
  filter(scenario == best.type) ->
  gcam_best
...
obs %>%
  filter(year == 1992) %>%
  select(GCAM_commodity, area) %>%
  rename(base.allocation = area) ->
  obs_base
obs %>%
  select(GCAM_commodity, area, year) %>%
  left_join(obs_base, by=c("GCAM_commodity")) %>%
  mutate(diff = area - base.allocation) ->
  obs

y.label <- expression(paste("Change in Area (thous ", km^2, ") from 1992"))
p <- ggplot() + geom_line(data=obs, aes(year, diff), color="black", size=1.5)
p <- p + geom_line(data=gcam_best, aes(year, diff, color=scenario), size=2)
p <- p + geom_line(data=gcam, aes(year, diff, color=scenario), size=0.5)
p <- p + xlab("Year") + ylab(y.label) + scale_color_manual(name="", values=ET_COLORS)
p <- p + facet_wrap(~GCAM_commodity, nrow=1, labeller = label_wrap_gen(width=15))
p <- p + theme(axis.title = element_text(size = 16, face="bold"),
               axis.text = element_text(size = 14),
...
```{r allland-ts-sm, echo=FALSE, message=FALSE, fig.width=10, fig.height=6, fig.cap="Area of all land types over time by Expectation Type. Black lines are observation data either from the CCI satellite (downloaded from FAOSTAT) or the FAO."}
best.type <- nrms[which.min(nrms$landTypeMeanObjFunVal), "expectation.type"]
best_output %>%
  filter(calibration.year == 1990,
         comparison.years == "1990-2015",
  ...
  mutate(scenario = sub("Best_", "", scenario)) %>%
  filter(scenario %in% SCENARIOS_TO_PLOT,
         # Remove biomass & willow -- the names appear in the output but they aren't actually turned on in these simulations
         !(GCAM_commodity %in% c("biomass", "willow"))) ->
  gcam
gcam %>%
  filter(scenario == best.type) ->
  gcam_best
obs_data %>%
  filter(region == "USA", 
         year %in% 1975:2015, 
         !(GCAM_commodity %in% c("Cropland", "Cropland_Harvested", "Forest"))) ->
  obs
obs %>% filter(source == "FAO") -> obs_fao
obs %>% filter(source == "CCI") -> obs_cci
y.label <- expression(paste("Area (thous ", km^2, ")"))
p <- ggplot() + geom_line(data=obs_fao, aes(year, area), color="black", size=1.5)
p <- p + geom_line(data=obs_cci, aes(year, area), color="gray45", size=1.5)
p <- p + geom_line(data=gcam_best, aes(year, land.allocation, color=scenario), size=2)
p <- p + geom_line(data=gcam, aes(year, land.allocation, color=scenario), size=0.5)
p <- p + xlab("Year") + ylab(y.label) + scale_color_manual(name="", values=ET_COLORS)
p <- p + facet_wrap(~GCAM_commodity, labeller = label_wrap_gen(width=15))
p <- p + theme(axis.title = element_text(size = 16, face="bold"),
               axis.text = element_text(size = 14),
               strip.text = element_text(size = 14),
               legend.text = element_text(size = 14))
p <-  p + scale_x_continuous(breaks=seq(1990, 2015, 10)) 
print(p)
ggsave("./4-figures/AllLand.png", height = 9, width = 14, units='in')
```


```{r save-results, echo=FALSE}
gcam %>%
  select(scenario, objfun, calibration.year, comparison.years, comparison.landTypes, assumption, GCAM_commodity, year, land.allocation, harvested.land) %>%
  rename(physical.land = land.allocation,
         expectation.type = scenario) ->
  gcam_save
write.csv(gcam_save, "./4-figures/all_land_results.csv", row.names = FALSE)
```
...
best_models %>%
  ...
  mutate(id = paste("Initial Model Year =", calibration.year)) ->
  calibration
...
```{r allcrop-pctdiff-sm, echo=FALSE, message=FALSE}
...
best_output %>%
  filter(calibration.year == 1990,
         ...
         assumption == "Default",
         objfun == "nrms") %>%
  filter(GCAM_commodity %in% ALL_CROPS_OBS) %>%
  filter(scenario %in% SCENARIOS_TO_PLOT) ->
  gcam
obs_data %>%
  filter(region == "USA", 
         GCAM_commodity %in% ALL_CROPS_OBS,
         year %in% 1975:2015) ->
  obs
...
comparison %>%
  group_by(GCAM_commodity, scenario) %>%
  summarize(min.diff = min(diff), max.diff = max(diff), med.diff = median(diff),
            min.pct.diff = min(pct.diff), max.pct.diff = max(pct.diff), med.pct.diff = median(pct.diff)) %>%
  ungroup() %>%
  mutate(`Error (million km2)` = paste(round(med.diff), " (", round(min.diff), " to ", round(max.diff), ")", sep=""),
         `Error (%)` = paste(round(med.pct.diff), " (", round(min.pct.diff), " to ", round(max.pct.diff), ")", sep="")) %>%
  select(scenario, GCAM_commodity, `Error (million km2)`, `Error (%)`) %>%
  rename(`gcamland land type` = GCAM_commodity,
         `Expectation type` = scenario) %>%
  arrange(`Expectation type`) ->
  table

table %>%
  left_join(crop_nrmse, by=c("gcamland land type" = "land.type", "Expectation type" = "expectation.type")) %>%
  rename(NRMSE = objfunval) %>%
  select(-landTypeMeanObjFunVal) %>%
  mutate(NRMSE = round(NRMSE, 2)) ->
  table

knitr::kable(table, caption="Relative and absolute error by crop and expectation type. Values indicate the median and range across years for each commodity and expectation type.")
```

```{r save-1990-info, echo=FALSE, message=FALSE}
# Select the default model
best_output %>%
  filter(calibration.year == 1990,
         comparison.years == "1990-2015",
  ...
  filter(year == 1990,
         scenario == "Adaptive") %>%
  mutate(calibratedProfit = expectedProfit * shareWeight) %>%
  select(scenario, year, GCAM_commodity, land.allocation, harvested.land, expectedProfit, calibratedProfit) %>%
  rename(`Physical land` = land.allocation,
         `Harvested land` = harvested.land,
         `Read In Profit` = expectedProfit,
         `Calibrated Profit` = calibratedProfit) ->
  data_1990
write.csv(data_1990, "./4-figures/1990_data.csv", row.names = FALSE)
```
